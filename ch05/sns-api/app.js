...
const cors = require('cors') // cors 미들웨어 -> ★api 서버는 반드시 설정해야 한다.
...
sequelize
   .sync({ force: false })
   .then(() => {
      console.log('데이터베이스 연결 성공') // 연결 성공
   })
   .catch((err) => {
...
app.use(
   cors({
      ...
      credentials: true, // 모든 쿠키, 세션 등 인증 정보 허용
   })
)
...
app.use(
   session({
      ...
      cookie: {
         ...
         secure: false, // http를 사용할 때만 쿠키 전송 여부 -> http, https 둘 다 사용 가능
      },
   })
)
