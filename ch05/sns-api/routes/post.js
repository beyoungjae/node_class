...
const upload = multer({
   ...
   storage: multer.diskStorage({
      ...
      filename(req, file, cb) {
         ...
         const ext = path.extname(decodedFileName) // 파일 확장자 추출

         const basename = path.basename(decodedFileName, ext)
         // 파일명 설정 : 기존 이름 + 업로드 날짜시간 + 확장자
         // ex) dog + 123214123 + .jpg
...
router.post('/', isLoggedIn, upload.single('img'), async (req, res) => {
   try {
      ...
      if (!req.file) {
         // 업로드된 파일이 없거나 문제가 생겨서 파일 정보가 넘어오지 않는 경우
       ...
       */
      const hashtags = req.body.hashtags.match(/#[^\s#]*/g) // #을 기준으로 해시태그를 추출
      ...
      if (hashtags) {
         ...
         HashTagInstance = {
          ...
          result = [
            [HashTagInstance1, true] // #여행 해시 태그가 새로 생성됨(false라면 이미 존재하는 해시태그)
          ...
          */
         // addHashtags(): HashTagInstance를 통해 post와의 관계를 설정하고
         // 이 과정에서 postHashtag테이블의 postId와 hashtagId 컬럼에 값이 추가가 된다
...
router.put('/:id', isLoggedIn, upload.single('img'), async (req, res) => {
   try {
      ...
      await post.update({
         ...
         img: req.file ? `/${req.file.filename}` : post.img, // 수정된 이미지 파일이 있으면 교체하고 없으면 기존 값 유지
      })
...
router.get('/', async (req, res) => {
   try {
      ...
      const limit = parseInt(req.query.limit, 10) || 3 // 한 페이지당 나타낼 레코드 개수 (기본값: 3)
      const offset = (page - 1) * limit // 오프셋 계산

      // 게시물의 레코드의 전체 개수 가져오기
      ...
      */
      const posts = await Post.findAll({
         limit,
         offset,
         order: [['createdAt', 'DESC']], // 최신 날짜 순으로 가져온다.
